    ...
    function applyBasicOp(inputImage, outputImage) {
        ...
        switch (currentBasicOp) {
            ...
            case "his-equal":
                ...
                var show_hist = $("#his-equal-show").prop("checked");
                var show_cdf = $("#his-cdf-show").prop("checked");
                var rand_hist = $("#his-equal-random").prop("checked");
                imageproc.histogram_equalizer(inputImage, outputImage, type, percentage, show_hist, show_cdf, rand_hist);
                break;
        }
    }
        ...
        if (currentShadeLayerOp != "no-op") {
            ...
            if (currentShadeLayerOp == "dither" &&
                ...
                for (var i = 0; i < shadeLayer.data.length; i+=4) {
                    if (shadeLayer.data[i] == shadeLayer.data[i+1]
                    &&  shadeLayer.data[i+1] == shadeLayer.data[i+2]
