(function(imageproc) {
    ...
    imageproc.comicColor = function(inputData, outputData, saturation) {
        ...
        for (var i = 0; i < inputData.data.length; i += 4) {
            ...
            let palette_index = 0;
            let min_palette_dist = 255 * 3;
            let min_palette_index = 0;
            palette.forEach((element) => {
                let dist = Math.hypot((element[0] - r), (element[1] - g), (element[2] - b));
                if(dist < min_palette_dist){
                    min_palette_dist = dist;
                    min_palette_index = palette_index;
                }
                palette_index++;
            });
            outputData.data[i]     = palette[min_palette_index][0];
            outputData.data[i + 1] = palette[min_palette_index][1];
            outputData.data[i + 2] = palette[min_palette_index][2];
        }
    }
